    ...
    switch (projectConfig.stackType) {
      case "SPRING":
        return {
          ...
          frontendConfig: projectConfig.frontendConfig,
      ...
      case "NODE":
        return { ...basePayload, nodeConfig: projectConfig.nodeConfig, frontendConfig: projectConfig.frontendConfig }
      case "NEST":
        return { ...basePayload, nestConfig: projectConfig.nestConfig, frontendConfig: projectConfig.frontendConfig }
      case "FASTAPI":
        return { ...basePayload, fastapiConfig: projectConfig.fastapiConfig, frontendConfig: projectConfig.frontendConfig }
      default:
        return { ...basePayload, frontendConfig: projectConfig.frontendConfig }
    }
  }
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select
                        value={projectConfig.springConfig.language}
                        onValueChange={(value: "java" | "kotlin") => setSpringConfig({ language: value })}
                      >
                        <SelectTrigger className="bg-input/50">
                          <SelectValue />
                        </SelectTrigger>
                  ...
                  <div>
                    <Label className="text-sm mb-1 block">Database Migration Tool</Label>
                    <Select
                      value={projectConfig.springConfig.migrationTool}
                      onValueChange={(value: "none" | "flyway" | "liquibase") => setSpringConfig({ migrationTool: value })}
                    >
                      <SelectTrigger className="bg-input/50">
                        <SelectValue />
                      </SelectTrigger>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {projectConfig.springConfig.migrationTool === "none"
                        ? "Schema managed by Hibernate ddl-auto"
                        : projectConfig.springConfig.migrationTool === "flyway"
                        ? "SQL-based migrations with versioned scripts"
                        : "XML/YAML-based migrations with changelog tracking"}
                    </p>
                  </div>

                  {/* Configuration Format */}
                  <div>
                    <Label className="text-sm mb-1 block">Configuration Format</Label>
                    <Select
                      value={projectConfig.springConfig.configFormat}
                      onValueChange={(value: "properties" | "yml") => setSpringConfig({ configFormat: value })}
                    >
                      <SelectTrigger className="bg-input/50">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="properties">application.properties</SelectItem>
                        <SelectItem value="yml">application.yml</SelectItem>
                      </SelectContent>
                    </Select>
                    <p className="text-xs text-muted-foreground mt-1">
                      {projectConfig.springConfig.configFormat === "properties"
                  ...
                  {/* Architecture Type */}
                  <div className="mt-4">
                    <Label className="text-sm mb-2 block">Architecture Type</Label>
                    <div className="grid grid-cols-2 gap-2">
                      {(["MONOLITH", "MICROSERVICES"] as const).map((type) => (
                        <button
                          key={type}
                          onClick={() => setSpringConfig({ architectureType: type })}
                          className={`p-3 rounded-lg border text-sm font-medium transition-all ${
                            projectConfig.springConfig.architectureType === type
                              ? "border-primary bg-primary/10 text-primary"
                              : "border-border bg-secondary/30 text-muted-foreground hover:border-primary/50"
                          }`}
                        >
                          {type === "MONOLITH" ? "🏢 Monolith" : "🔧 Microservices"}
                        </button>
                      ))}
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">
                      {projectConfig.springConfig.architectureType === "MONOLITH"
                        ? "Single deployable application"
                        : "Multi-module Spring Cloud project with service discovery"}
                    </p>
                  </div>

                  {/* Microservices Config */}
                  {projectConfig.springConfig.architectureType === "MICROSERVICES" && (
                    <div className="space-y-3 p-4 rounded-lg bg-secondary/20 border border-border">
                      <Label className="text-sm font-medium">Microservices Configuration</Label>
                      <div className="grid grid-cols-2 gap-2">
                        {(["AUTO", "MANUAL"] as const).map((mode) => (
                          <button
                            key={mode}
                            onClick={() => setMicroservicesConfig({ mode })}
                            className={`p-2 rounded-lg border text-xs font-medium transition-all ${
                              projectConfig.springConfig.microservicesConfig.mode === mode
                                ? "border-primary bg-primary/10 text-primary"
                                : "border-border bg-secondary/30 text-muted-foreground hover:border-primary/50"
                            }`}
                          >
                            {mode === "AUTO" ? "Auto (1 service per entity)" : "Manual grouping"}
                          </button>
                        ))}
                      </div>
                      {projectConfig.springConfig.microservicesConfig.mode === "AUTO" && tables.length > 0 && (
                        <div className="text-xs text-muted-foreground p-2 rounded bg-secondary/30">
                          <p className="font-medium mb-1">Services to be generated:</p>
                          {tables.filter(t => !t.isJoinTable).map(t => (
                            <span key={t.id} className="inline-block mr-2 mb-1 px-2 py-0.5 rounded bg-primary/10 text-primary">
                              {t.name.toLowerCase().replace(/s$/, "")}-service
                            </span>
                          ))}
                        </div>
                      )}
                      {projectConfig.springConfig.microservicesConfig.mode === "MANUAL" && (
                        <div className="text-xs text-muted-foreground p-2 rounded bg-secondary/30">
                          <p>Define service groups in the generated configuration after download.</p>
                        </div>
                      )}
                    </div>
                  )}
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold">Frontend Generation</h2>
            </div>
            <Switch
              checked={projectConfig.frontendConfig.enabled}
              onCheckedChange={(checked) => setFrontendConfig({ enabled: checked })}
            />
          </div>
          {projectConfig.frontendConfig.enabled && (
            <div className="space-y-4">
              <div>
                <Label className="text-sm mb-2 block">Framework</Label>
                <div className="grid grid-cols-3 gap-2">
                  {([
                    { id: "NEXTJS" as const, name: "Next.js", available: true },
                    { id: "ANGULAR" as const, name: "Angular", available: false },
                    { id: "REACT" as const, name: "React", available: false },
                  ]).map((fw) => (
                    <button
                      key={fw.id}
                      onClick={() => fw.available && setFrontendConfig({ framework: fw.id })}
                      disabled={!fw.available}
                      className={`p-3 rounded-lg border text-sm font-medium transition-all ${
                        projectConfig.frontendConfig.framework === fw.id && fw.available
                          ? "border-primary bg-primary/10 text-primary"
                          : fw.available
                            ? "border-border bg-secondary/30 text-muted-foreground hover:border-primary/50"
                            : "border-border bg-secondary/10 text-muted-foreground/50 cursor-not-allowed"
                      }`}
                    >
                      {fw.name}
                      {!fw.available && <span className="block text-[10px] mt-0.5">Coming soon</span>}
                    </button>
                  ))}
                </div>
              </div>
              <p className="text-xs text-muted-foreground">
                Generates a complete Next.js frontend with TypeScript, Tailwind CSS, and CRUD pages for all your entities.
              </p>
            </div>
          )}
        </motion.div>
